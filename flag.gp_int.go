...
type IntFlag struct {
	...
	////////////////////////////////////////////////////////////////////////////
	//area for parsing
	target *int          // target value pointer(maybe new(int) if Target not set)
	info   impl.FlagInfo // parsed info of this flag
}

// init verify and init the flag info
func (f *IntFlag) init(namegen *util.NameGenenerator) error {
	f.info.Flag = f
...
func (f *IntFlag) validateValues(values int) error {
	return f.validValue(values)
}

// check if value if valid for this flag
func (f *IntFlag) validValue(value int) error {
	if len(f.Enums) > 0 {
		found := false
		for _, v := range f.Enums {
			if value == v {
...
// // Int looks up the value of a local IntFlag
...
var _ impl.Flag = (*IntFlag)(nil) //for interface verification only
